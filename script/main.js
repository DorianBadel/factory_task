...
function setInitialGalleryState() {
  ...
  */
	...
	sliderGalleryBottom.append(
		...Array.from(sliderGalleryBottom.childNodes).reverse()
	);

	const galleryTop = document.querySelector(".slider__gallery--top").innerHTML;
	const galleryBottom = document.querySelector(
		".slider__gallery--bottom"
	).innerHTML;
...
function calculateWidthTop() {
	let sum = 0;
	imagesTop.forEach((img) => (sum += img.width + paddingTop));
	return sum;
}

function calculateWidthBottom() {
	let sum = 0;
	imagesBottom.forEach((img) => (sum += img.width + paddingBottom));
	return sum;
}
...
function moveRowBack() {
	...
	if (focusIdTop == imagesTop.length) {
		...
		if (focusIdTop > 1) {
			...
			sliderGalleryTop.style.transform = `translateX(-${
				arrayWidthTop - offsetTop
			}px)`;
			focusIdTop++;
		} else {
	...
	if (focusIdBottom == imagesBottom.length) {
		...
		if (focusIdBottom > 1) {
			...
			sliderGalleryBottom.style.transform = `translateX(-${
				arrayWidthBottom - offsetBottom
			}px)`;
			focusIdBottom++;
		} else {
			offsetBottom +=
				imagesBottom[Math.abs(focusIdBottom) - 1].width / 2 + paddingBottom;
			sliderGalleryBottom.style.transform = `translateX(-${
				arrayWidthBottom - offsetBottom
			}px)`;
